...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const collaborator = await respondToCollaboration(id, user.brokerId, true);

    await writeAuditLog({
      actorUserId: user.id,
      action: "DEAL_COLLABORATION_ACCEPTED",
    ...
    if (error instanceof Error) {
      return apiError("COLLABORATION_ACCEPT_FAILED", error.message, 409);
    }
    const { status, body } = toErrorResponse(error);
    return Response.json(body, { status });
  }
