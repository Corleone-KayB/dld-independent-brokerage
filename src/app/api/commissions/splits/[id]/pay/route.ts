import { markSplitPaid } from "@/modules/commission-splits/service";
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const split = await markSplitPaid(id, user.brokerId);

    await writeAuditLog({
      actorUserId: user.id,
      action: "COMMISSION_SPLIT_PAID",
    ...
    if (error instanceof Error) {
      return apiError("COMMISSION_SPLIT_PAY_FAILED", error.message, 409);
    }
    const { status, body } = toErrorResponse(error);
    return Response.json(body, { status });
  }
