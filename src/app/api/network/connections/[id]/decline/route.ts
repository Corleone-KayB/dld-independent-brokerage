import { declineConnection } from "@/modules/network/service";
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const connection = await declineConnection(id, user.brokerId);

    await writeAuditLog({
      actorUserId: user.id,
      action: "NETWORK_CONNECTION_DECLINED",
    ...
    if (error instanceof Error) {
      return apiError("CONNECTION_DECLINE_FAILED", error.message, 409);
    }
    const { status, body } = toErrorResponse(error);
    return Response.json(body, { status });
  }
