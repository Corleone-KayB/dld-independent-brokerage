...
import { listComplianceDocuments, documentExpiryState } from "@/modules/compliance/service";
import { Card } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { VerificationBadge } from "@/components/compliance/verification-badge";
import { UploadDocumentForm } from "@/components/compliance/upload-document-form";
...
export default async function CompliancePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl font-semibold text-charcoal">Compliance</h1>
        <p className="mt-1 text-charcoal/60">
          Track your required documents and their verification status.
        </p>
      </div>

      <UploadDocumentForm partnerId={user.partnerId} />

      {documents.length === 0 ? (
        <Card className="p-10 text-center text-charcoal/50">No documents uploaded yet.</Card>
      ) : (
              ...
              <Card key={doc.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
                <div>
                  ...
                  <p className="text-xs text-charcoal/50">
                    {doc.type.replace("_", " ")} · {doc.expiresAt ? `Expires ${formatDate(doc.expiresAt)}` : "No expiry"}
                  </p>
                </div>
